...
![без денормализации](./images/db-logic.png)

### 5.1.2 Схема денормализованная

![денормализованная](/images/db-logic-denorm.png)
...
| :----- | :------: | :---------: | :-------: | :-----------------: | :-------------------: | --------: |
| 8 байт | 256 байт |  1024 байт  | 512 байт  |       8 байт        |       256 байт        | 2064 байт |
...
| Id     |  title   | description | cover_src | tracks_count | duration |     Итого |
| :----- | :------: | :---------: | :-------: | :----------: | :------: | --------: |
| 8 байт | 256 байт |  1024 байт  | 512 байт  |   4 байта    | 4 байта  | 1808 байт |
...
| Id     | user_id | track_id | track_title | cover_src | track_album | track_artist | duration | src_320  | src_loseless | is_relevant |     Итого |
| :----- | :-----: | :------: | :---------: | :-------: | :---------: | :----------: | :------: | :------: | :----------: | :---------: | --------: |
| 8 байт | 8 байт  |  8 байт  |  256 байт   | 512 байт  |  256 байт   |   256 байт   | 4 байта  | 512 байт |   512 байт   |   1 байт    | 2333 байт |

### 5.2.10 TrackHistory

| Id     | user_id | track_id |  time  |  score  | cover_src | track_album | track_artist |  title   | duration |   Итого    |
| :----- | :-----: | :------: | :----: | :-----: | :-------: | :---------: | :----------: | :------: | :------: | :--------: |
| 8 байт | 8 байт  |  8 байт  | 8 байт | 2 байта | 512 байт  |  256 байт   |   256 байт   | 256 байт | 4 байта  | 1318 байта |

### 5.2.11 Track

| Id     |  Title   | cover_src | duration | src_320  | src_loseless | album_names | artist_names |     Итого |
| :----- | :------: | :-------: | :------: | :------: | :----------: | :---------: | :----------: | --------: |
| 8 байт | 256 байт | 512 байт  | 4 байта  | 512 байт |   512 байт   |  512 байт   |   512 байт   | 2828 байт |



### 5.2.12 QPC

| Таблица             | Действие |  QPC  |                                                                  Комментарий |
| :------------------ | :------: | :---: | ---------------------------------------------------------------------------: |
| User                |  Чтение  |  333  |                                                                              |
|                     |  Запись  |   -   |                                                    Относительно мало записей |
| Session             |  Чтение  |  333  |                                                                              |
|                     |  Запись  |   -   |                                                    Относительно мало записей |
| Artist              |  Чтение  |  423  |                                                                        Поиск |
|                     |  Запись  |   -   |                                                                              |
| TrackArtist         |  Чтение  |  423  |                                                                        Поиск |
|                     |  Запись  |   -   |                                                                              |
| Album               |  Чтение  |  423  |                                                                        Поиск |
|                     |  Запись  |   -   |                                                                              |
| AlbumTrack          |  Чтение  |  423  |                                                                        Поиск |
|                     |  Запись  |   -   |                                                                              |
| Playlist            |  Чтение  | 1178  |                                                                              |
|                     |  Запись  |   -   |                                         Пользователи редко создают плейлисты |
| PlaylistTrack       |  Чтение  |  845  |                                                                              |
|                     |  Запись  |  333  |                                                                              |
| TrackHistory        |  Чтение  |  80   |                             В основном читается при перестройке рекомендаций |
|                     |  Запись  | 19000 |                               Слушают часто, постоянно запись о прослушенных |
| ReccomendationQueue |  Чтение  | 16150 | Планируется, что сервисом рекомендаций пользуется не менее 85% пользователей |
|                     |  Запись  |  66   |                Раз в день перестраиваем рекоммендации активных пользователей |
| Track               |  Чтение  | 1689  |                                                                              |
...
| ReccomendationQueue |        user_id, track_id         |                          Равномерное |
