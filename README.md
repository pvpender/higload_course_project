...
![схема](./images/db-new-denorm.png)
...
| Session             |   Postgres + Redis    |   Только для Redis   |     -      | PG: 1 master + 2 replica; Redis: 3 master + 3 slave |         Redis кеширует недавние сессии, все сессии в PG |
...
| ReccomendationQueue | Postgres + ClickHouse | 16 шардов по user_id |     -      |                  1 реплика на шард                  |                   Наиболее загружена при денормализации |
| Track               |       Postgres        |          -           |     -      |                1 master + 2 replica                 |              При денормализации нагрузка сильно снижена |
| File                |       Postgres        |          -           |     -      |                1 master + 2 replica                 |         Почти не используется в денормализованной схеме |
| UserFeedback        | Postgres + ClickHouse |          -           |     -      |                1 master + 2 replica                 | Малая нагрузка на запись, используется для рекомендаций |
...
| ReccomendationQueue |   IDX(id), IDX(user_id, trakc_id), IDX(user_id, genre), IDX(user_id, is_relevant)   |
...
| :---------- | ---------------------------: |
| PostgreSQL  | PG_DUMP раз в сутки + RAID 5 |
